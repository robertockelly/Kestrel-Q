#ifndef KQ_STATUS_H
#define KQ_STATUS_H

#include <stddef.h>

#ifdef __cplusplus
extern "C" {
#endif

typedef enum kq_status {
    KQ_STATUS_OK = 0,
    KQ_STATUS_INVALID_ARGUMENT,
    KQ_STATUS_OUT_OF_MEMORY,
    KQ_STATUS_FILE_OPEN_FAILED,
    KQ_STATUS_FILE_SIZE_FAILED,
    KQ_STATUS_FILE_MAP_FAILED,
    KQ_STATUS_TRUNCATED,
    KQ_STATUS_BAD_MAGIC,
    KQ_STATUS_UNSUPPORTED_VERSION,
    KQ_STATUS_MALFORMED_METADATA,
    KQ_STATUS_UNSUPPORTED_METADATA,
    KQ_STATUS_MALFORMED_TENSOR,
    KQ_STATUS_UNSUPPORTED_TENSOR_TYPE,
    KQ_STATUS_ARITHMETIC_OVERFLOW,
    KQ_STATUS_LIMIT_EXCEEDED,
    KQ_STATUS_INVALID_ALIGNMENT,
    KQ_STATUS_SPAN_OUT_OF_RANGE,
    KQ_STATUS_DUPLICATE_METADATA,
    KQ_STATUS_DUPLICATE_TENSOR,
    KQ_STATUS_INCONSISTENT_DATA_SECTION,
    KQ_STATUS_UNSUPPORTED_MODEL,
    KQ_STATUS_MODEL_TOPOLOGY_MISMATCH,
    KQ_STATUS_SEMANTIC_MAPPING_FAILED,
    KQ_STATUS_UNKNOWN_PHYSICAL_TENSOR,
    KQ_STATUS_NO_TENSOR_PAYLOAD,
    KQ_STATUS_INVALID_QUANTIZED_GEOMETRY,
    KQ_STATUS_NONCONTIGUOUS_TENSOR_VIEW,
    KQ_STATUS_TENSOR_LAYOUT_MISMATCH,
    KQ_STATUS_TENSOR_OWNERSHIP_MISMATCH,
    KQ_STATUS_INCOMPATIBLE_TOKENIZER,
    KQ_STATUS_INVALID_UTF8,
    KQ_STATUS_INVALID_TOKEN_ID,
    KQ_STATUS_BUFFER_TOO_SMALL,
    KQ_STATUS_UNSUPPORTED_TOKENIZER_OPTION,
    KQ_STATUS_UNSUPPORTED_CHAT,
    KQ_STATUS_MALFORMED_CHAT,
    KQ_STATUS_INCOMPATIBLE_PLE,
    KQ_STATUS_INVALID_PLE_STATE,
    KQ_STATUS_MALFORMED_QUANTIZED_DATA,
    KQ_STATUS_NUMERIC_DOMAIN,
    KQ_STATUS_DIMENSION_MISMATCH,
    KQ_STATUS_ALIASING_VIOLATION,
    KQ_STATUS_INCOMPATIBLE_GDN,
    KQ_STATUS_INVALID_GDN_STATE,
    KQ_STATUS_INCOMPATIBLE_QSA,
    KQ_STATUS_INVALID_QSA_STATE,
    KQ_STATUS_INCOMPATIBLE_MOE,
    KQ_STATUS_INCOMPATIBLE_PLE_VALUE,
    KQ_STATUS_INVALID_PLE_VALUE_STATE,
    KQ_STATUS_PLE_LOOKUP_FAILED,
    KQ_STATUS_INCOMPATIBLE_LAYER,
    KQ_STATUS_INVALID_LAYER_STATE,
    KQ_STATUS_INCOMPATIBLE_MODEL_EXEC,
    KQ_STATUS_INVALID_MODEL_STATE,
    KQ_STATUS_INCOMPATIBLE_SAMPLING,
    KQ_STATUS_INVALID_RNG_STATE
} kq_status;

#define KQ_DIAGNOSTIC_CAPACITY 256U

typedef struct kq_diagnostic {
    kq_status status;
    char message[KQ_DIAGNOSTIC_CAPACITY];
} kq_diagnostic;

const char *kq_status_string(kq_status status);
void kq_diagnostic_clear(kq_diagnostic *diagnostic);

#ifdef __cplusplus
}
#endif

#endif
